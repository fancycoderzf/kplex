...
int numColour[MaxN], colour[MaxN], upper[MaxN];
...
void init() {
    ...
    memset(numColour, 0, (N+1)*sizeof(int));
...
void expand(vector<int> P) {
    ...
    for(int i = tmp-1; i >= 0; --i) {
        int v = P[i];
        if(C.size() + upper[v] - numColour[colour[v]] <= maxSize || C.size() + P.size() <= maxSize)
            return ;
...
void output() {
    ...
    std::ofstream ansOfs("result/kplex_Col/" + graphName + "_ans_" + KK + ".txt");

    ansOfs << "{";
    ansOfs << "algorithm: kplex_Col, name: " << graphName << ", K: " << K << ", ";
